// StoryTime-Frontend/components/userStories.tsx
import { useEffect, useState } from "react";
import { Button } from "./ui/button";
import { CardHorizontal } from "./ui/card";
import Image from "next/image";
import { getUserStories, deleteStory } from "@/api/storyApi"; // Import deleteStory
...
export default function UserStories() {
  ...
  const handleDeleteStory = async (storyId: string) => {
    if (window.confirm("Are you sure you want to delete this story?")) {
      try {
        ...
        // Refresh the list of stories after deletion
  ...
  const handleEditStory = (storyId: string) => {
    // Navigate to the story's main page, where chapters can be managed/edited
    router.push(`/book/${storyId}`);
  };

  const handleApprove = async (storyId: string, chapterIndex: number) => {
    try {
      console.log("Approving chapter", chapterIndex, "for story", storyId);
            ...
            >
              ...
              <div className="flex-1 pr-0 sm:pr-4">
                ...
                <div className="flex flex-wrap gap-2 mt-4">
                  <Button onClick={() => handleEditStory(story._id)} className="bg-black text-white hover:bg-gray-900">Edit</Button>
                  <Button onClick={() => handleDeleteStory(story._id)} variant="destructive">Delete</Button>
                </div>
